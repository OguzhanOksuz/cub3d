...
/*   Updated: 2023/07/10 19:19:31 by mkaraden         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void calculate_perpetual_and_color(Game *game, t_ray *ray, double angle)
{
	...
	if (ray->side == 0)										 // East or west wall.
		ray->color = ray->ray_dir.x > 0 ? 0x00FF0000 : 0x0000FF00; // Red for east, green for west.
	else													 // North or south wall.
		ray->color = ray->ray_dir.y > 0 ? 0x000000FF : 0x00FFFF00; // Blue for north, yellow for south.
...
void raycast(Game *game)
{
	...
	for (int x = 0; x < WIDTH; x++, angle += angle_step)
	{
		...
		while (ray.hit == 0)
		{
			...
			else if (map[ray.map_y][ray.map_x] > 0)
			{
				ray.hit = 1;
			}
		}
